...
**Response**: The PASSING `med_budget`, `med_domgross`, and `med_intgross` were all lower then the fail row. 
...
**Response**: The indicator with the highest `med_budget`, `med_domgross`, and `med_intgross` were the `notalk`, where no women talk to each other. 
...
**Response**: I would say there is not statistical significance in the different pas fail and Bechdel variables. But it is interesting that all the medians are about the same. 
